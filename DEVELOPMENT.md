...
src/
  ...
  levels/            # 关卡数据
    ...
    levelValidator.ts # 关卡校验工具（开发期用）
  ai/                # AI 技能生成
    skillGenerator.ts # 调用 Claude API 生成技能
    prompt.ts         # prompt 模板
    skillCache.ts     # 局内技能缓存
  data/              # 静态数据
    taunts.ts         # 对手吐槽文案库
  types/             # 全局类型定义
    index.ts
...
type GameState = {
  ...
  phase: "selecting" | "playerTurn" | "aiTurn" | "cardPicking" | "won" | "lost";
  levelId: number;
...
interface BoardAPI {
  ...
  requestArea(center: Pos, range: number): Promise<Pos>;
}
```

---

## Phase 1：最小可玩（纯五子棋 + 固定技能）

**目标**：不接 AI，用手写技能跑通完整的一局游戏。
...
在 `data/testSkills.ts` 中硬编码：

```typescript
// 釜底抽薪：移除一颗指定的黑子
const fuDiChouXin: Skill = {
  name: "釜底抽薪",
...
```
function playerTurn():
  1. if turnNumber > 0 && turnNumber % 3 === 0 → 触发抽卡
...
**CardPicker.tsx**：三选一浮层。三张卡并排，第三张标注"自定义"。Phase 1 中三张都是从手写技能池随机抽的，"自定义"暂时也从池里抽。

**OpponentDialog.tsx**：棋盘上方的气泡。显示对手头像 + 一句话。3 秒后自动消失。

### 1.9 关卡数据

Phase 1 写 5 个关卡，在 `levels/levels.ts` 中定义。
...
**目标**：接入 Claude API，实现成语 → 技能的实时生成。

### 2.1 设置页

- SettingsPanel.tsx：输入 OpenRouter API Key，存 localStorage
- 提供模型选择下拉框，默认使用 `anthropic/claude-sonnet-4`，也可选其他模型
- 启动时检查是否有 Key，无则引导设置
...
- BoardAPI 的完整接口说明（10 个方法的签名和语义）
- 约束规则（最多影响 5 格、不能直连五子、不能全清黑子）
...
- 前两张卡：系统随机选两个成语 → 调用 `generateSkill` → 显示
- 第三张卡：显示输入框，玩家输入成语 → 调用 `generateSkill`
- 生成中显示加载状态
- 三张卡同时并行生成，全部完成后才展示选择

### 2.6 成语词库

在 `data/idioms.ts` 中内置约 200 个有画面感的四字成语（偏动作、军事、自然类，适合转化为棋盘操作）。系统从中随机抽取供三选一的前两张。
...
**目标**：30+ 关卡，完善 AI 对手，关卡进度。

### 3.1 关卡扩充

- 用脚本预生成 30-50 关，写入 `levels/levels.ts`
- 每关有验证（无五连、有解）
- 分 5 个章节，每章 6-10 关
...
- 扩充文案库到每类 10-20 条
- 根据关卡章节切换对手人格（前 10 关是菜鸟、中间是正常对手、后期是高手口吻）
...
技能执行中的 `requestChoice` 和 `requestArea` 是异步操作（需要等待用户交互）。用 Promise 实现：调用 requestChoice 时 UI 进入选择模式，用户点击后 resolve Promise，技能函数继续执行。

### 错误边界

技能沙盒执行用 try-catch 包裹。任何运行时错误（AI 生成的代码有 bug）都 catch 住，跳过该技能，手牌退回，游戏继续。控制台打印错误信息用于调试。
