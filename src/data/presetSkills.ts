...
export const PRESET_SKILLS: Skill[] = [
  ...
  },
  {
    ...
    executeCode: `
      ...
      const sorted = blacks.sort((a, b) => {
        const distA = Math.max(Math.abs(a.row - 7), Math.abs(a.col - 7));
        const distB = Math.max(Math.abs(b.row - 7), Math.abs(b.col - 7));
        return distB - distA;
      });
      api.transform(sorted[0].row, sorted[0].col, "white");
    `,
  },
  {
    name: "声东击西",
  ...
  },
  {
    ...
    executeCode: `
      const empty = api.findEmpty();
      // Prefer center area (rows/cols 5-9)
  ...
  },
  {
    ...
    executeCode: `
      ...
      if (blacks.length === 0) return;
      const size = api.getBoardSize();
      // Find black stone with fewest black neighbors
  ...
  },
  {
    ...
    executeCode: `
      ...
      let prev = { row: target.row, col: target.col };
  ...
  },
  {
    ...
    executeCode: `
      ...
      if (empty.length === 0) return;
      const size = api.getBoardSize();
      // Find empty cell near most blacks
      let bestScore = -1;
      let bestPos = empty[0];
      for (const pos of empty) {
        let score = 0;
